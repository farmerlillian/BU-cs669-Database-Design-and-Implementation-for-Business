...
alter table account add column total_pomos integer not null default 0;
alter table account add column remain_draw_attempts integer not null default 0;

create table luckydraw(
	present_id decimal(12) not null primary key,
	present_name varchar(16) not null,
	present_value decimal(5,2) not null,
	probability decimal(5,4) not null
);

create table drawrecord(
record_id decimal(12) not null primary key,
account_id decimal(12) not null references account(account_id),
present_id decimal(12) not null references luckydraw(present_id),
date DATE not null
);

create sequence luckydraw_seq;
create sequence drawrecord_seq;

-- Create a trigger on table focus to update the total_pomos in table Account
CREATE OR REPLACE FUNCTION update_account_total_pomos()
RETURNS TRIGGER AS $$
DECLARE
	v_account_name VARCHAR(16);
    v_total_pomos INTEGER;
BEGIN
 	SELECT account_name INTO v_account_name FROM Account 
	WHERE account_id = (SELECT account_id from Period WHERE period_id=New.period_id);
    -- Calculate the total count of Focus rows with is_one_pomo equal to 1 for every user
    SELECT sum(today_pomos) into v_total_pomos 
	FROM GoalAchievement 
	group by account_name
	having account_name=v_account_name;
    -- Update the total_pomos field in the relevant group of the AccountGroup table
    UPDATE Account SET total_pomos = v_total_pomos WHERE account_name = v_account_name;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Create a trigger to fire when inserting new rows into the Focus table
CREATE TRIGGER focus_insert_trigger2
AFTER INSERT ON Focus
FOR EACH ROW
EXECUTE FUNCTION update_account_total_pomos();

--Create a trigger on table Account to update the remain_draw_attempts in table Account
CREATE OR REPLACE FUNCTION update_remain_draw_attempts()
RETURNS TRIGGER AS $$
BEGIN
    -- 检查是否需要增加 remain_draw_attempts
    IF NEW.total_pomos % 10 = 0 THEN
        UPDATE Account
    	SET remain_draw_attempts = remain_draw_attempts + 1;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER update_draw_attempts_trigger
AFTER UPDATE OF total_pomos ON Account
FOR EACH ROW
EXECUTE FUNCTION update_remain_draw_attempts();

--create a trigger on drawrecord to update the remain_draw_attempts in table account
CREATE OR REPLACE FUNCTION update_remain_draw_attempts()
RETURNS TRIGGER AS $$
BEGIN
    -- 找到对应账户的 remain_draw_attempts，并减少 1
    UPDATE Account
    SET remain_draw_attempts = remain_draw_attempts - 1
